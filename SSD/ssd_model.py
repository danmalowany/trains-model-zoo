...
class HeadsExtractor(nn.Module):
    def __init__(self, backbone):
        ...
        def split_backbone(net):
            features_extraction = [x for x in net.named_children()][:-2]

            if type(net) == torchvision.models.vgg.VGG:
                features_extraction = [*features_extraction[0][1]]
                net_till_conv4_3 = features_extraction[:-8]
                till_conv5_3 = features_extraction[-7:-1]
                rest_of_net = features_extraction[-1]
            elif type(net) == torchvision.models.resnet.ResNet:
                net_till_conv4_3 = features_extraction[:-2]
                till_conv5_3 = [features_extraction[-2]]
                rest_of_net = [features_extraction[-1]]
            else:
                raise ValueError('We only support VGG and ResNet backbones')
            # return nn.Sequential(*net_till_conv4_3), nn.Sequential(*rest_of_net)
            return nn.Sequential(OrderedDict(net_till_conv4_3)), nn.Sequential(OrderedDict(till_conv5_3)), \
                   nn.Sequential(OrderedDict(rest_of_net))
        ...
        self.till_conv4_3, self.till_conv5_3, self.rest_of_net = split_backbone(backbone)
        ...
        self.conv6_2 = nn.Conv2d(1024, 1024, kernel_size=3, padding=6, dilation=6)
        self.conv7_2 = nn.Conv2d(2048, 512, kernel_size=1)

        from trains import Task
        configuration_data = Task.current_task().get_model_config_dict()
        self.OPTION = configuration_data.get('option', 4)
        print('SSD option = {}'.format(self.OPTION))
        configuration_data['option'] = self.OPTION
    ...
    def forward(self, x):
        ...
        #if 'layer1' in [child[0] for child in self.till_conv4_3.named_children()]:
        if self.OPTION == 1:
            h = self.till_conv5_3(h)
            hs.append(h)

            h = self.rest_of_net(h)
            ...
            if h.shape[1] == 512:
                ...
                raise ValueError('We need to add a conv6 for input shape {}'.format(h.shape))
            h = F.relu(self.conv7(h))
            hs.append(h)  # conv7
...
class SSD(nn.Module):
    ...
    def _create_heads(self):
        ...
        for i in range(len(self.in_channels)):
            self.loc_layers += [nn.Sequential(*[nn.Dropout2d(p=self.dropout),
                                                nn.Conv2d(self.in_channels[i], self.num_anchors[i] * 4,
                                                          kernel_size=3, padding=1)])]
            self.cls_layers += [nn.Sequential(*[nn.Dropout2d(p=self.dropout),
    ...
    def update_priors(self, priors_generator):
        updated_priors_boxes, updated_priors, updated_anchors = priors_generator(self.fm_sizes, self.steps)
        self.box_coder = SSDBoxCoder(self.steps, self.box_sizes, self.aspect_ratios, self.fm_sizes,
                                     box_generator=lambda steps, box_sizes, aspect_ratios, fm_sizes:
                                     (updated_priors_boxes, updated_priors))
        self.num_anchors = updated_anchors
        self._create_heads()
